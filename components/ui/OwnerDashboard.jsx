...
export default function OwnerDashboard() {
  ...
  async function submit(event) {
    ...
    setState("saving");
    const response = await fetch("/api/checkout", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ productType: "MEMORIAL", memorialData: form }) });
    const data = await response.json();
    if (data.url) {
      window.location.assign(data.url);
      return;
    }
    setState(data.error || "Plaćanje nije moguće pokrenuti.");
  }
  ...
  async function addDemo() {
    ...
    setState(data.error || "Demo memorijal nije moguće dodati.");
  }
